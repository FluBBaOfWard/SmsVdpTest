...
.define CYCLE_TEST_IDX0	$c81c
.define CYCLE_TEST_IDX1	$c81e
.define CYCLE_TEST_IDX2	$c820
...
;==============================================================
.sdsctag 1.31,"SMSVDPTest","Test the SMS VDP","FluBBa"
...
VDP_ScreenOn:
	ld a,%01100000				;$60=vblank irq + screen on.
	jp VDP_SetReg1
.ends
...
;==============================================================
RunTest91:					;Show screen on/off latch time.
;==============================================================
	di							; disable interrupts
	call VDP_ScreenOff
	call StdOut_Cls
...
;	ld a,$03					;7cyc. NameTableBase value
	ld a,$10					;7cyc. Hbl IRQ, SMS mode off
	out (VDP_Ctrl),a			;11 cyc
	ld a,$80					;7cyc. VDP Reg 0
	out (VDP_Ctrl),a			;11 cyc, HCount second 0xF3, cycle after latch.
	...
	ld de,($C000)				;20cyc
	in a,(VDP_Ctrl)				;11 cyc
	ld a,$14					;7cyc. Vbl IRQ, SMS mode on
	out (VDP_Ctrl),a			;11 cyc
	ld a,$80					;7cyc. VDP Reg 0
	out (VDP_Ctrl),a			;11 cyc, HCount first 0xF3, cycle before latch.
...
;==============================================================
RunTest200:					; Test cyles per op-code.
;==============================================================
	di							; disable interrupts
	call VDP_ScreenOff
	call StdOut_Cls
...
+:
	...
	push de
	ld hl,t200_return
	push hl						; push return adress to stack
	ld b,b
	ld hl,timingTable
	ld a,(CYCLE_TEST_IDX0)
	add a,a
	add a,l
	ld l,a
	ld a,$00
...
t200_return
	...
	ld a,$FF					;7cyc. Flip HL in/out so HCounter gets latched.
	out (IO_Ctrl),a				;11cyc. Probably around 0x56
	in a,(HCounter)				;11cyc. Read HCounter value, 0x27 for NOP
	cp a,$27
	jr nz, t200_error
...
;==============================================================
; Test all CPU op-codes timing.
;==============================================================
.section "CPU timing" FREE

opcode_00:
	.db $00						; NOP
	ret
opcode_01:
	.db $01, $00, $00			; LD BC,#0000
	ret
opcode_02:
	.db $01, $00, $00			; LD (BC),A
	ret
timingTable:
	.dw opcode_00,opcode_01,opcode_02
.ends
...
cycles_text:
	.db $20,$7F," Testing Op-codes cycletime",$0a
	.db $20,$7F," Please wait...",$0a
	.db $00
